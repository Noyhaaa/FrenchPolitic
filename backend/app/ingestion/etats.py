...
Elle pose au passage la **source Légifrance** des lois promulguées : le seul
...
from app.ingestion.navette import etat_du_texte, source_legifrance
...
async def _main(dry_run: bool, legislature: int) -> None:
    ...
    async with session_factory() as session:
        for row in (await session.execute(select(DossierRow))).scalars().all():
            ...
            dossier.etat = etat
            loi = source_legifrance(etat)
            if loi is not None and all(s.url != loi.url for s in dossier.sources):
                dossier.sources = [*dossier.sources, loi]
                sources_posees += 1
    ...
    if total:
        ...
        print(f"\n{sources_posees} source(s) Légifrance ajoutée(s).")
